...
from llama_pipe import LlamaRMSNormPipe, LmHeadPipe


class EmbeddingPipe(nn.Module):
    def __init__(self, loader_util, orig, use_flash_attention_2, sliding_window):
        super().__init__()
        self.orig = orig
...
class MixtralForCausalLMPipe(PipelineModel, transformers.MixtralForCausalLM):
    ...
    def to_layer_specs(self):
        ...
        result = [
            initial_layer,
            LayerSpec(EmbeddingPipe, self.loader_util, self.model.embed_tokens, self.model._use_flash_attention_2, self.model.config.sliding_window),
        ]
